...
if __name__ == '__main__':
    ...
    if args.eiglog is not None and args.eiglog > 0:
        ...
        m = re.search(
            r'(?:(?<=^)|(?<=[\s,]))eiglog(?:[:=](\d+))?(?=\b)', args.tags.lower())
        if m is not None:
            ...
            m2 = re.search(r'eiglog(?:[:=]?(\d+))?',
                           str(args.modeldesc).lower())
            if m2 is not None:
    ...
    eigenvalue_log_interval = eiglog_interval if (
        eiglog_interval is not None and eiglog_interval > 0) else (log_interval * 2)
    ...
    if not debug:
        if args.modeldesc is not None:
            ...
            modeldesc = f"erf_cubic_P_{num_samples}_D_{input_size}_N_{hidden_size}_epochs_{epochs}_lrA_{lrA:.2e}_time_{timestamp}"
    ...
    # Projections for eigen-analysis across d coordinates
    ...
    with tqdm(total=epochs, desc="Training", unit="epoch", initial=epoch) as pbar:
        ...
        while epoch < epochs:
            ...
            if epoch == 0 or initialized == False:
                ...
                with torch.no_grad():
                    try:
                        gpr_pred = gpr_dot_product_explicit(
                            X, Y.squeeze(), X, 1.0)
                        model_out = model(X)
                        if model_out.ndim == 3:
                    ...
                    try:
                        ...
                        # Nicely formatted printout of all eigenvalues and mus
                        try:
     ...
     # Log to TensorBoard
                        writer_cm.add_scalars(
                            'Eigenvalues/He1', scalarsH1, epoch)
                        writer_cm.add_scalars(
                            'Eigenvalues/He3', scalarsH3, epoch)
                        writer_cm.add_scalars(
                            'Eigenvalues/K_He1', scalarsK1, epoch)
                        writer_cm.add_scalars(
                            'Eigenvalues/K_He3', scalarsK3, epoch)

                        # Log learnabilities (mu) for K eigenvalues
                        writer_cm.add_scalars(
                            'Learnability/He1_mu', muK1, epoch)
                        writer_cm.add_scalars(
                            'Learnability/He3_mu', muK3, epoch)
                        eigenvalues = {
                            'He1': scalarsH1,
                            'He3': scalarsH3,
                            'K_He1': scalarsK1,
                ...
                try:
                    ...
                    if not torch.isfinite(loss):
                        print(
                            f"Warning: Invalid loss at epoch {epoch}: {loss.item()}")
                        pbar.update(1)
                        epoch += 1
                        continue
                    ...
                    print(
                        f"[Warning] Google Sheets logging failed at first epoch: {gsheets_exc}")
            # Clear gradients
            model.zero_grad()
                ...
                with torch.no_grad():
                    for i, param in enumerate(model.parameters()):
                        if param.grad is not None:
                            ...
                            if param.grad is not None:
                                param.add_(param.grad, alpha=-
                                           effective_learning_rate_for_update)
            ...
            if ((epoch + 1) % eigenvalue_log_interval == 0 or epoch == 0) and not args.debug:
                with torch.no_grad():
                    ...
                    # Log to TensorBoard
                    writer_cm.add_scalars('Eigenvalues/He1', scalarsH1, epoch)
                    writer_cm.add_scalars('Eigenvalues/He3', scalarsH3, epoch)
                    writer_cm.add_scalars(
                        'Eigenvalues/K_He1', scalarsK1, epoch)
                    writer_cm.add_scalars(
                        'Eigenvalues/K_He3', scalarsK3, epoch)
                    ...
                    # Log learnabilities (mu) for K eigenvalues
                    writer_cm.add_scalars('Learnability/He1_mu', muK1, epoch)
                    writer_cm.add_scalars('Learnability/He3_mu', muK3, epoch)
                    ...
                    if (epoch + 1) % detailed_log_interval == 0 or epoch == 0:
                        ...
                        with torch.no_grad():
                            ...
                            cos_sim = torch.dot(model_out_flat, gpr_pred_flat) / (
                                torch.norm(model_out_flat) * torch.norm(gpr_pred_flat) + 1e-12)
                            writer_cm.add_scalar(
                                'CosineSimilarity/Model_vs_GPR', cos_sim.item(), epoch)
