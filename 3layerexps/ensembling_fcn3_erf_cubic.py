...
if __name__ == '__main__':
    ...
    parser.add_argument('--eps', type=float, default=0.01, help='Small coefficient for He3 component in target')
    ...
    eps = float(getattr(args, 'eps', 0.01))
    ...
    Y3_inf = X_inf**3 - 3.0 * X_inf  # He3 along each coordinate
            ...
            if epoch == 0 or initialized == False:
                ...
                with torch.no_grad():
                    ...
                    try:
                        ...
                        # Print initial eigenvalues summary for He1
                        if lH1.shape[0] > 1:
                            lH10 = lH1[0].item() if hasattr(lH1[0], 'item') else float(lH1[0])
                            lH1_rest = lH1[1:]
                            lH1_rest_mean = lH1_rest.mean().item() if hasattr(lH1_rest.mean(), 'item') else float(lH1_rest.mean())
                            lH1_rest_std = lH1_rest.std().item() if hasattr(lH1_rest.std(), 'item') else float(lH1_rest.std())
                            print(f"Initial He1 eigenvalues: lH1[0]={lH10:.4g}, mean(lH1[1:])={lH1_rest_mean:.4g}, std(lH1[1:])={lH1_rest_std:.4g}")
                        else:
                            print(f"Initial He1 eigenvalue: lH1[0]={lH1[0].item() if hasattr(lH1[0], 'item') else float(lH1[0])}")

                        scalarsH1 = {}
                        scalarsH3 = {}
                        d_plot = min(int(lH1.shape[0]), 10)
                        for i in range(d_plot):
                            scalarsH1[str(i)] = lH1[i].item() if hasattr(lH1[i], 'item') else float(lH1[i])
                            scalarsH3[str(i)] = lH3[i].item() if hasattr(lH3[i], 'item') else float(lH3[i])
                        eigenvalues = { 'He1': scalarsH1, 'He3': scalarsH3 }
                    except Exception:
                        pass
