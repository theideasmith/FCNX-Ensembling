...
include(joinpath(@__DIR__, "FCS_Hermite.jl"))
using .FCS_Hermite
...
function parse_commandline()
    s = ArgParseSettings(description="FCS Hermite Eigenvalue Solver with Target and Perpendicular modes")
...
function main()
    ...
    if !quiet
        println("#####################################################################")
        println("##    &*&       THE GREAT FCN3-ERF HERMITE EOS SOLVER   &*&.        ")
        println("##                       Initializes                                 ")
        println("#####################################################################")
        println()
        println("Parameters:")
    ...
    exp_sol_target = FCS_Hermite.nlsolve_solver(
        i0_target,
    ...
    l1_target, l3_target = FCS_Hermite.compute_lK_ratio(exp_sol_target, P, n1, n2, χ, d, δ_target, κ, ϵ, b)
    ...
    if exp_sol_target === nothing
        ...
        lJ1T = lJ3T = lH1T = lH3T = lWTT = nothing
        lK1T = lK3T = nothing
    else
        lJ1T, lJ3T, lH1T, lH3T, lWTT = exp_sol_target
        lV1T = -(lH1T / lJ1T^2 - 1 / lJ1T)
        TrH = lH1T + lH3T + (d - 1) * (15 * (1/d)^3) + (d - 1) * 1/d
        print(" TrH: $(sf(TrH)), beta: $(sf((3.0 / sqrt(6.0)) * (TrH - 1.0)))")
        if !quiet
            println("lH1T: $(sf(lH1T)), lH3T: $(sf(lH3T))")
            println("lJ1T: $(sf(lJ1T)), lJ3T: $(sf(lJ3T))")
            @printf("lWTT: %.*g\n", 7, lWTT)
            println("lJ1T/lWT: $(lJ1T/(b*lWTT))")
            println("lJ3T/lWT^3: $(lJ3T/(15*16/(27*π) * lWTT^3))")
            println()
        ...
        lK1T, lK3T = FCS_Hermite.compute_lK(exp_sol_target, P, n1, n2, χ, d, δ_target, κ, ϵ, b)
    ...
    exp_sol_perp = FCS_Hermite.nlsolve_solver(
        i0_perp,
    ...
    l1_perp, l3_perp = FCS_Hermite.compute_lK_ratio(exp_sol_perp, P, n1, n2, χ, d, δ_perp, κ, ϵ, b)
    ...
    if exp_sol_perp === nothing
        ...
        lJ1P = lJ3P = lH1P = lH3P = lWTP = nothing
        lK1P = lK3P = nothing
    else
        lJ1P, lJ3P, lH1P, lH3P, lWTP = exp_sol_perp
        ...
        if !quiet
            ...
            @printf("lWTP: %.*g\n", 7, lWTP)
            println("lJ1P/lWP: $(lJ1P/(b*lWTP))")
            println("lJ3P/lWP^3: $(lJ3P/(15*16/(27*π) * lWTP^3))")
            println()
        ...
        lK1P, lK3P = FCS_Hermite.compute_lK(exp_sol_perp, P, n1, n2, χ, d, δ_perp, κ, ϵ, b)
