...
def load_model(run_dir: Path, config: Dict, device: torch.device) -> Optional[FCN3NetworkEnsembleLinear]:
...
def compute_empirical_lH(
    model: FCN3NetworkEnsembleLinear, d: int, device: torch.device
...
def compute_empirical_spectrum(
    model: FCN3NetworkEnsembleLinear, d: int, P: int, device: torch.device, k: Optional[int] = None
) -> Optional[np.ndarray]:
