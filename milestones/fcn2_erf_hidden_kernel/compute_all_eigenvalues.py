...
def main():
    ...
    theory_lJ_eff = []
    theory_lJP_eff = []
    results_by_config = {}  # Group results by (d, P, N, chi) to compute dataset averages
    ...
    with open("theory_target.json", "r") as f:
        theory_data_target = f.read()
    theory_result_target = json.loads(theory_data_target)
    target_lJ = theory_result_target["lJ"]
    print(f"Theoretical target eigenvalue at P=600, N=600, d=100, chi=80.0, kappa=1.0: {target_lJ:.6f}")

    plt.errorbar(xvals, yvals, yerr=ystds, fmt='o', color='b', alpha=0.5, capsize=3)
    plt.axhline(y=target_lJ, color='r', linestyle='--', label='Theoretical Target Eigenvalue')
    ...
    # Add dataset-averaged plot (mean with stderr)
    if results_by_config:
        ...
        plt.errorbar(config_xvals, config_means, yerr=config_sterrs, fmt='s', color='g', 
                    alpha=0.7, capsize=5, label='Dataset average (mean +/- stderr)', markersize=8, linewidth=2)
