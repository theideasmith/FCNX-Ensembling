#!/usr/bin/env python3
"""
Training script for 2-layer linear network with eigenvalue tracking.
...
Usage:
    python train_fcn2_linear.py --d 10 --P 30 --N 100 --epochs 2000000 --device cuda:0
"""
...
def train_fcn2_linear(d, P, N, epochs=2_000_000, log_interval=10_000, 
    ...
    """Train 2-layer linear network and track H eigenvalues.
    ...
    # Setup directory
    if run_dir is None:
        run_dir = Path(__file__).parent / f"linear_d{d}_P{P}_N{N}_lr_{lr}_T_{temperature}_chi_{chi}"
    run_dir.mkdir(exist_ok=True, parents=True)
    
    print(f"\nTraining 2-layer linear network:")
    print(f"  d={d}, P={P}, N={N}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train 2-layer linear FCN2")
