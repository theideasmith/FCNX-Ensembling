...
def train_and_track(d, P, N, epochs=300_000_000, log_interval=10_000, device_str="cuda:1", writer=None, from_checkpoint: bool = False):
    ...
    # Setup directories
    base_name = f"d{d}_P{P}_N{N}_chi{N}"
    parent_dir = Path(__file__).parent
    base_run_dir = parent_dir / base_name
    run_dir = parent_dir / (base_name + ("_from_checkpoint" if from_checkpoint else ""))
    ...
    # In from-checkpoint mode, load from base directory but save into suffixed run_dir
    load_dir = base_run_dir if from_checkpoint else run_dir
    model_checkpoint = load_dir / "model.pt"
    if not model_checkpoint.exists():
        alt = load_dir / "model_final.pt"
        if alt.exists():
            model_checkpoint = alt
    start_epoch = 0
    ...
    if model_checkpoint.exists():
        ...
        # Only resume logs if not forking from checkpoint
...
def main():
    ...
    if args.d is not None:
        ...
        # Setup TensorBoard
        tb_suffix = "_from_checkpoint" if args.from_checkpoint else ""
        tensorboard_dir = Path(__file__).parent / "runs" / f"d{d}{tb_suffix}"
        writer = SummaryWriter(log_dir=str(tensorboard_dir))
        print(f"TensorBoard logging to: {tensorboard_dir}")
        
        final_eigs, eigs_over_time, used_run_dir = train_and_track(
            d, P, N, epochs=epochs, log_interval=log_interval,
            device_str=args.device, writer=writer, from_checkpoint=args.from_checkpoint
        )
        
        writer.close()
        
        # Generate prediction plot
        plot_predictions_vs_empirical(d, P, N, used_run_dir)
