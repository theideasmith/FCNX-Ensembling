#!/usr/bin/env python3
"""Red-robin launcher for FCN3-erf P-sweep experiments."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Red-robin launcher for FCN3-erf MF P-sweep")
    ...
    parser.add_argument("--out-dir", type=str, default="results_fcn3_erf", help="Output subdirectory")
    args = parser.parse_args()

    chi = float(args.N if args.chi is None else args.chi)
    p_values = build_p_values(args.d, args.p_max, args.num_p)
    train_script = os.path.join(os.path.dirname(__file__), "d_sweep_fcn3_erf.py")
