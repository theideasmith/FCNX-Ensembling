...
# DIMS_DEFAULT = [2, 6, 8, 10, 15, 18, 20]
...
POLL_INTERVAL = 5.0  # seconds
DIMS_DEFAULT = [2, 6, 8, 10, 15, 18, 20, 150]
DEVICE_DEFAULT = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
...
def find_run_dirs(base: Path, dims: List[int]) -> List[Tuple[Path, Dict[str, int]]]:
    ...
    selected: List[Tuple[Path, Dict[str, int]]] = []
    for d in dims:
        N = 6 * d if d > 150 else 50
        name = f"d{d}_P{3*d}_N{N}_chi{N}"
        p = base / name
        if p.is_dir():
            selected.append((p, {"d": d, "P": 3*d, "N": N, "chi": N}))
    # Sort by d
    selected.sort(key=lambda x: x[1]["d"]) 
    return selected
...
def compute_theoretical_loss(d: int, kappa: float = 1.0, chi: float = 700, P: int = 600) -> float:
    if d > 150:
        N = 6 * d
    else:
        N = 50
...
def compute_empirical_loss(model: FCN3NetworkActivationGeneric, d: int, device: torch.device, num_samples: int = 1000) -> Optional[float]:
    try:
        with torch.no_grad():
            P = 3 * d
            torch.manual_seed(42)
            X = torch.randn(P, d, device='cuda:1')
            y = X[:,0].unsqueeze(-1)  # Target is first input feature
            y_pred = model(X)
            diff = y_pred - y  # (P, ensemble)
        ...
        print(f"Warning: failed to compute empirical loss: {e}")
        return None
...
def update_plot_loss(
    d_vals: List[int],
    emp_loss: List[Optional[float]],
    th_loss: List[Optional[float]],
    out_path: Path,
) -> None:
    """Plot loss vs d on log-log scales."""
    d_arr = np.array(d_vals, dtype=float)
    emp_loss_arr = np.array([v if v is not None else np.nan for v in emp_loss], dtype=float)
    th_loss_arr = np.array([v if v is not None else np.nan for v in th_loss], dtype=float)

    fig, ax = plt.subplots(figsize=(11, 7))

    # Plot empirical loss
    ax.loglog(d_arr, emp_loss_arr, 'o-', color='tab:blue', linewidth=2.5, markersize=8,
              label="Empirical Loss")
    
    # Plot theoretical loss
    ax.loglog(d_arr, th_loss_arr, 's--', color='tab:red', linewidth=2.0, markersize=7,
              label="Theoretical Loss")

    ax.set_xlabel("Dimension $d$ (log scale)")
    ax.set_ylabel("Loss (log scale)")
    ax.set_title("Empirical and Theoretical Loss vs $d$")
    ax.grid(True, which='both', alpha=0.3)
    ax.legend(loc='best', fontsize=9)
...
def poll_and_plot_once(
    base_dir: Path, device: torch.device, dims: List[int]
) -> Tuple[List[int], List[Optional[float]], List[Optional[float]], List[Optional[float]], List[Optional[float]], List[Optional[float]], List[Optional[float]], List[Optional[float]], List[Optional[float]]]:
    """Poll models and collect eigenvalue data.
    ...
    - th_top_list: theoretical top eigenvalues (lHT)
    - th_perp_list: theoretical perpendicular eigenvalues (lHP)
    - emp_loss_list: empirical losses
    - th_loss_list: theoretical losses
    """
    runs = find_run_dirs(base_dir, dims)
    print(runs)
    d_list: List[int] = []
    ...
    th_top_list: List[Optional[float]] = []
    th_perp_list: List[Optional[float]] = []
    emp_loss_list: List[Optional[float]] = []
    th_loss_list: List[Optional[float]] = []
    loss_ratio_list: List[Optional[float]] = []
    for run_dir, cfg in runs:
        cfg = load_config_from_name(cfg)
        d = cfg["d"]
        model = load_model(run_dir, cfg, device)
        ...
        mean_top, std_top, mean_rest, std_rest = compute_empirical_lH(model, d, device)
        lHT, lHP = compute_theoretical_lH(run_dir, d, device)
        loss_th = compute_theoretical_loss(d)
        loss_emp = compute_empirical_loss(model, d, device)
        ratio = (loss_emp / loss_th) if (loss_emp is not None and loss_th is not None and loss_th != 0) else None
        loss_ratio_list.append(ratio)
        # Additionally, compute and save per-run spectrum plots via randomized SVD
        spectrum_np = compute_empirical_spectrum(model, d, cfg["P"], device)
        preds_obj = compute_theoretical_full_predictions(cfg, device)
        if spectrum_np is not None:
        ...
        th_top_list.append(lHT)
        th_perp_list.append(lHP)
        emp_loss_list.append(loss_emp)
        th_loss_list.append(loss_th)
    
    return d_list, emp_top_list, emp_top_err_list, emp_rest_list, emp_rest_err_list, th_top_list, th_perp_list, emp_loss_list, th_loss_list, loss_ratio_list

def update_plot_loss_ratio(
    d_vals: List[int],
    loss_ratio: List[Optional[float]],
    out_path: Path,
) -> None:
    """Plot loss ratio vs d on log-log scales."""
    d_arr = np.array(d_vals, dtype=float)
    loss_ratio_arr = np.array([v if v is not None else np.nan for v in loss_ratio], dtype=float)

    fig, ax = plt.subplots(figsize=(11, 7))

    # Plot loss ratio
    ax.loglog(d_arr, loss_ratio_arr, 'o-', color='tab:purple', linewidth=2.5, markersize=8,
              label="Empirical/Theoretical Loss Ratio")
    
    ax.set_xlabel("Dimension $d$ (log scale)")
    ax.set_ylabel("Loss Ratio (log scale)")
    ax.set_title("Empirical/Theoretical Loss Ratio vs $d$")
    ax.grid(True, which='both', alpha=0.3)
    ax.legend(loc='best', fontsize=9)
    ...
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
    print(f"Saved loss ratio plot to {out_path}")
def main():
    base_dir = Path(__file__).resolve().parent
    out_path = base_dir / "plots" / "poll_lH_vs_d.png"
    loss_out_path = base_dir / "plots" / "poll_loss_vs_d.png"
    slopes_path = base_dir / "plots" / "slope_history.json"
    slopes_plot_path = base_dir / "plots" / "slopes_over_time.png"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    device = DEVICE_DEFAULT
    ...
    try:
        while True:
            d_vals, emp_top, emp_top_err, emp_rest, emp_rest_err, th_top, th_perp, emp_loss, th_loss, loss_ratio = poll_and_plot_once(
                base_dir, device, DIMS_DEFAULT
            )
            if d_vals:
                # Sort by d for clean plotting
                ...
                th_top = list(np.array(th_top, dtype=object)[order])
                th_perp = list(np.array(th_perp, dtype=object)[order])
                emp_loss = list(np.array(emp_loss, dtype=object)[order])
                th_loss = list(np.array(th_loss, dtype=object)[order])
                loss_ratio = list(np.array(loss_ratio, dtype=object)[order])
                slope_emp_top, slope_th_top, slope_emp_rest, slope_th_perp = update_plot(
                    d_vals, emp_top, emp_top_err, emp_rest, emp_rest_err, th_top, th_perp, out_path
                )
                loss_ratio_plot_path = base_dir / "plots" / "poll_loss_ratio_vs_d.png"
                update_plot_loss(d_vals, emp_loss, th_loss, loss_out_path)
                update_plot_loss_ratio(d_vals, loss_ratio, loss_ratio_plot_path)
                # Append slopes if changed from last measurement
                def maybe_append(hist: List[float], val: Optional[float], tol: float = 1e-6) -> bool:
                    if val is None or np.isnan(val):
                        return False
