...
test_P_ld = 2000
seed_ld = 42
N_PER_D = None  # if set, N = chi = N_PER_D * d (overrides N_ld)
...
def N_chi_for(d):
    """Hidden width and χ. Default N=400; with N_PER_D set, N=χ=N_PER_D·d."""
    if N_PER_D:
        N = int(N_PER_D) * int(d)
        return N, float(N)
    return int(N_ld), float(N_ld)


def scaling_for_d(d, chi=None):
    if chi is None:
        _, chi = N_chi_for(d)
    t_eff = T_ld / float(chi)
    sigma_w0 = s0_ld / d
    wd_W0 = (1.0 / sigma_w0) * t_eff
    return sigma_w0, wd_W0, t_eff
...
def sweep_basename(suffix):
    return f"langevin_mf_{sweep_stem()}_P_sweep_ep{EPOCHS_SWEEP_LD}{suffix}"
...
def langevin_tag(d, P, epochs=None, N=None, chi=None):
    ...
    return (
        ...
        f"_lr{base_lr_ld}_T{T_ld}_eps{eps_ld}_seed{seed_ld}_ep{epochs}"
    )
...
def train_langevin_mf_one(d, P, device, epochs=None, log_every=None):
    ...
    N, chi = N_chi_for(d)
    sigma_w0, wd_W0, t_eff = scaling_for_d(d, chi=chi)
    sigma_a_run = 1.0 / (N * chi)
    wd_A_run = N * t_eff * chi
    ...
    print(
        ...
        f"sigmaW0={sigma_w0:.4g}  wd_W0={wd_W0:.4g}  device={device}",
        flush=True,
    )
...
def vga_cache_path(d, P):
    return os.path.join(OUT_DIR, f"{langevin_tag(d, P)}_vga.json")
...
def vga_he3_for_P(d, P, kappa, advanced=True):
    ...
    if os.path.exists(cache_path):
        ...
        if not kappa_ok or cached_adv != bool(advanced):
            os.remove(cache_path)
    payload = _run_julia_theory(cmd, cache_path, "VGA", d, P, kappa)
    if payload is None:
        return float("nan")
...
def plot_all_d(by_d, P_list, show=False, vga_by_d=None, class_by_d=None):
    ...
    for i, d in enumerate(sorted(by_d)):
        ...
        ax[0].plot(P_arr, he3_ld, "o-", color=color, lw=2, label=f"Langevin d={d}")
        ax[0].plot(P_arr, he3_gpr, "s--", color=color, lw=1.4, alpha=0.85, label=f"GPR d={d}")
        ...
        ax[1].plot(P_arr, mse_ld, "o-", color=color, lw=2, label=f"Langevin d={d}")
        ax[1].plot(P_arr, mse_gpr, "s--", color=color, lw=1.4, alpha=0.85, label=f"GPR d={d}")
    ax[0].set_xscale("log")
    ax[0].set_xlabel(r"$P$")
    ax[0].set_ylabel("test He3")
    ax[0].set_title("cubic learnability vs $P$")
    ax[0].grid(True, which="both", alpha=0.3)
    ax[0].legend(fontsize=6.5, ncol=2, loc="lower right")
    ax[1].set_xscale("log")
    ax[1].set_xlabel(r"$P$")
    ax[1].set_ylabel("test MSE")
    ax[1].set_title("test MSE vs $P$")
...
COLLAPSE_ALPHAS = (1.0, 1.5, 2.0, 2.5, 3.0)


def plot_collapse(by_d, P_list, show=False, alphas=COLLAPSE_ALPHAS, vga_by_d=None, class_by_d=None):
    """He3 vs P / d^alpha, one panel per alpha, stacked."""
    n = len(alphas)
    fig, axes = plt.subplots(n, 1, figsize=(7.4, 2.35 * n), sharey=True)
    dims = sorted(by_d)
    P_arr = np.asarray(P_list, dtype=float)
    for ax, alpha in zip(np.atleast_1d(axes), alphas):
        for i, d in enumerate(dims):
            hists, gpr_rows = by_d[d]
            x = P_arr / (float(d) ** alpha)
            he3_ld = np.asarray([hists[P]["he3_test"][-1] for P in P_list])
            he3_gpr = np.asarray([r[1] for r in gpr_rows])
        ...
        ax.set_xlabel(rf"$P / d^{{{alpha:g}}}$")
    fig.suptitle("cubic learnability collapse", y=1.01, fontsize=12)
    fig.tight_layout()
...
def main(dims=None, P_list=None, show=False, plot_only=False, train_only=False):
    ...
    n_desc = (
        f"N={N_PER_D}*d (chi=N)" if N_PER_D else f"N={N_ld} chi={chi_ld:g}"
    )
    print(
        f"{n_desc} T={T_ld} lr0={base_lr_ld}  "
    ...
    if by_d:
        vga_by_d = {}
        class_by_d = {}
        print("VGA cubic learnability (fcn2_vga_erf.jl, kappa=kappa_eff)", flush=True)
        for d in sorted(by_d):
