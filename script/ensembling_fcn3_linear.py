...
def gen_eigenvalues_for_logging(model, X_inf, Y1_inf):
    with torch.no_grad():
        ...
        # U = torch.matmul(Q, Ut)
        # _ = Sigma.diagonal() / X_inf.shape[0]
...
def sift_eigenvalues_for_logging(lH1, lK1, kappa, num_samples):
        ...
        return scalarsH1, scalarsK1, muK1
    

def nicely_print_eigen_summary(lH1, lK1, scalarsH1, scalarsK1, muK1):
    try:
        ...
        header = f"{'idx':>3} {'He1':>14} {'K_He1':>14} {'mu_He1':>10}"
        print(header)
        print("-" * len(header))
        ...
        for i in range(d_print):
            ...
            print(
                f"{i:3d} {_num(h1):14.6g} {_num(k1):14.6g} {_num(mu1):10.6g}")
...
def log_eigenvalues_to_tensorboard(writer, epoch, scalarsH1, scalarsK1, muK1):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lrA', type=float, default=1e-5,
    ...
    parser.add_argument('--experiment_dirname', type=str, default='ensembling_fcn3_linear',
                        help='Base directory name for experiments (default: ensembling_fcn3_linear)')
    ...
    parser.add_argument('--data_averaged_num', type=int, default=None,
                        help='If provided, modify DATA_SEED by this number to enable random dataset indexing (N)')
    ...
    hidden_size = args.N
    input_size = args.d
    lrA = args.lrA / num_samples  # Base learning rate scaled by P
    output_size = 1
    ...
    log_interval = 1_000
    save_interval = 1_000
    eigenvalue_log_interval = 1_000
    gsheets_log_interval = 100_000
    tqdm_log_interval = 1

    timestamped = getattr(args, 'timestamped', False)
    experiment_dirname = getattr(args, 'experiment_dirname', 'ensembling_fcn3_linear')   
    ...
    DATA_SEED = 613
    # Modify DATA_SEED by data_averaged_num if provided
    data_averaged_num = getattr(args, 'data_averaged_num', None)

    if data_averaged_num is not None:
        DATA_SEED = DATA_SEED + data_averaged_num
    MODEL_SEED = 26
    LANGEVIN_SEED = 480
    ...
    if not debug and not ghost:
        if args.modeldesc is not None:
            ...
            save_dir = os.path.join("/home/akiva/exp/fcn3lin", modeldesc)
            ...
            if timestamped == True:
                ...
                modeldesc = f"linear_P_{num_samples}_D_{input_size}_N_{hidden_size}_epochs_{epochs}_lr_{args.lrA:.2e}_time_{timestamp}"
            else:
                modeldesc = f"linear_P_{num_samples}_D_{input_size}_N_{hidden_size}_epochs_{epochs}_lr_{args.lrA:.2e}"
        ...
        debug_tmp_dir = os.path.join(
            '/home/akiva/exp/fcn3lin/debugtmp', str(uuid.uuid4()))
    ...
    # Target: y(x) = He1(w·x) with w = e1 (purely linear)
    ...
    try:
        lH1, lK1 = gen_eigenvalues_for_logging(
            model, X_inf, Y1_inf)
        scalarsH1, scalarsK1, muK1 = sift_eigenvalues_for_logging(
            lH1, lK1, kappa, num_samples)
        nicely_print_eigen_summary(
            lH1, lK1, scalarsH1, scalarsK1, muK1)

    except Exception as e:
        print('Failed to compute initial eigenvalues:',  e)
        pass

    # MF predictions not yet implemented for linear network
    ...
    while epoch < epochs:
        ...
        eigenvalues = {}
        if ((epoch) % eigenvalue_log_interval == 0 or epoch == 0) and not ghost:
            try:
                lH1, lK1 = gen_eigenvalues_for_logging(
                    model, X_inf, Y1_inf)
                eigenvalues_data = sift_eigenvalues_for_logging(
                    lH1, lK1, kappa, num_samples)
                scalarsH1, scalarsK1, muK1 = eigenvalues_data
