...
NUM_DATASETS = 7  # Train on 7 different datasets
...
def build_command(d, lr, dataset_num, extra_args=None, headless=False):
    ...
    cmd = [
        ...
        "--device", str(device),
        "--ens", str(5),
        "--data_averaged_num", str(dataset_num),
        "--experiment_dirname", f"d_sweep_linear_dataset_{dataset_num}",
    ]
    if headless:
        cmd += ["--headless"]
    if extra_args:
...
def run_job(d, lr, dataset_num, dry_run=False, extra_args=None, headless=False):
    ...
    log_path = LOG_DIR / f"d_sweep_linear_d_{d}_lr_{lr:.0e}_ds_{dataset_num}.log"
    attempted = [cmd]
