""""
DEPRECATED – NN Training file from Noa which I modified for my purposes
"""
...
SAVE_PATH = "/home/akiva/FCN2_Test/network_ensemble"
NUM_NETS = 20
...
max_epochs = 50000#00000#300000#300000
lr0 = 1e-3
n_train,N,d = 400,200,50
n_test = 400
chi = N
sw2, sa2, s2 = 1.0/50.0, 1/200,(1/N*chi)
FL_scale = 1 #float(128)
activation = "lin"
eps = 0.
...
class MyNetwork():
    ...
    def __init__(self, input_dim, num_channels,n_train,n_test,max_epochs,sigma_2,sigma_a2,sigma_w2,FL_scale,eps,seeds,activation,save_path,lr0):
        ...
        self.activation = activation
        self.net = FCN_2_layers(self.d,self.N,1.0/(self.N**2),1.0/self.d,self.activation).to(DEVICE)
        ...
        self.temperature = 2*self.s2*FL_scale/(self.N)
        self.wd_input = self.temperature*self.d/self.sw2 
        self.wd_output = self.temperature/(self.sa2*FL_scale/(self.N**2))
...
for k in range(NUM_NETS):
    ...
    np.random.seed()
    net = MyNetwork(d,N,n_train,n_test,max_epochs,s2,sa2,sw2,FL_scale,eps,[train_seed,test_seed],activation,SAVE_PATH,lr0)
    net.train_net(k)
    print("done with ",k)
