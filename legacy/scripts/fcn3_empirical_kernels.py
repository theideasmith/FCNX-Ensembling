...
if __name__ == '__main__':
    ...
    output_tensor = torch.empty(num_batches_dim1, num_batches_dim2, num_samples_per_network, output_feature_dim).to(hp.DEVICE)
    ...
    for i, network_info in enumerate(networks_loader):
        loaded_count += 1
        raw_X = network_info.get('data').to(hp.DEVICE)[:160]
        raw_Y = network_info.get('target').to(hp.DEVICE)[:160]
        P =raw_X.shape[0]
        ...
        with torch.no_grad():
            ...
            plot_filename =f"../plots/fcn3/fcn3_model_output_comparison_{i}.png"
